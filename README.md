...
**🔑 가장 먼저 할 일 — Google "Sync Select" 페이지**

1. PC/모바일 브라우저로 접속: **https://calendar.google.com/calendar/syncselect**
2. 본인 Google 계정 로그인 상태에서, 추가한 캘린더(예: `기상청 날씨`) 체크박스 **ON**
3. **[Save]** 클릭
4. 폰에서 Google 캘린더 앱을 완전히 종료한 뒤 재실행 → 잠시 후 반영됨

> ⚠️ 이 페이지가 모바일 동기화의 **마스터 스위치**입니다. 여기서 체크돼 있지 않으면 폰에선 절대 안 보여요.

**OS별 추가 확인**

| 환경 | 방법 |
|---|---|
| **Android — Google 캘린더 앱** | 좌측 ☰ → 설정 → 해당 캘린더 → **"동기화" 토글 ON** + 좌측 ☰ 캘린더 목록에서 체크박스 ON |
| **삼성 갤럭시 — Samsung 캘린더 앱** | ⚠️ 삼성 캘린더는 구독 캘린더를 표시하지 못 합니다. **Google 캘린더 앱을 설치해서 거기서 보세요.** |
| **iPhone — 권장(가장 안정적)** | 구글 거치지 말고 Apple 캘린더에 **직접 구독**: 설정 → 캘린더 → 계정 → 계정 추가 → 기타 → 구독 캘린더 추가 → 서버에 Raw URL |
| **iPhone — Google 계정 경유** | Sync Select 마친 뒤 **iOS용 Google 캘린더 앱**에서 확인. iPhone 기본 캘린더 앱에는 안 보입니다 (CalDAV가 구독 캘린더 미동기화) |

**그래도 안 보일 때 점검 순서**

| 순서 | 확인할 것 |
|---|---|
| 1 | PC 웹 Google 캘린더에서 보이는가? → 안 보이면 Sync Select / 구독 자체 문제 |
| 2 | https://calendar.google.com/calendar/syncselect 에서 체크 ON 되어 있나? |
| 3 | 폰 Google 캘린더 앱 설정 → 해당 캘린더 → "동기화" + "표시" 둘 다 ON? |
| 4 | 좌측 메뉴 캘린더 목록의 체크박스 ON? |
| 5 | 최대 24시간 기다림. 그래도 안 되면 캘린더 삭제 → 재구독 |

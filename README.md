...
**선택 시크릿 (없으면 해당 기능만 비활성, 워크플로우는 정상 동작)**

| Name | 효과 | 값 예시 |
|---|---|---|
| `DATA_GO_KR_KEY` | 🌫️ 미세먼지/초미세먼지 등급 표시 (오늘+다음날 SUMMARY에 이모지 추가) | [공공데이터포털](https://www.data.go.kr/) 일반 인증키 |
| `AIR_REGION` | 미세먼지 예보 지역명 **오버라이드** (기본은 `REG_ID_LAND`/`REG_ID_TEMP` 에서 자동 추정) | `서울`, `경기북부`, `경기남부`, `강원영서`, `제주` 등 |
| `WRN_KEYWORDS` | 🚨 기상특보 필터 — 이 키워드를 포함한 지역만 표시 (없으면 전국 경보만) | `서울,경기,수도권` |
| `WRN_INCLUDE_WATCH` | 주의보까지 포함할지 (기본: 경보만) | `1` 로 설정하면 주의보 포함 |

> 💡 **AIR_REGION 자동 추정 동작**:
> - `11B00000` (수도권) → `서울` (서울 외 인천·경기 사용자는 명시 오버라이드 권장)
> - `11D10000` → `강원영서`, `11D20000` → `강원영동`
> - `11C10000` → `충북`, `11C20000` → `충남`
> - `11F10000` → `전북`, `11F20000` → `전남`
> - `11G00000` → `제주`, `11H10000` → `경북`, `11H20000` → `경남`
> - `REG_ID_TEMP` 코드로 광역시(대전/세종/광주/대구/부산/울산/인천) 자동 인식 시도
